  ...
  flake.modules.nixos.system =
    ...
    in
    {
      environment.systemPackages = [
        custom.keyboard-add
        custom.keyboard-remove
      ];

      services.udev.extraRules = ''
        ACTION=="add", ATTRS{idVendor}=="3297", ATTRS{idProduct}=="1977", RUN+="${pkgs.keyboard-remove}"
        ACTION=="remove", ATTRS{idVendor}=="3297", ATTRS{idProduct}=="1977", RUN+="${pkgs.keyboard-add}"
      '';
    };
